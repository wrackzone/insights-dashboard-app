...
Ext.define('CustomApp', {
    ...
    launch: function() {
		...
		app.metrics = [
            "TimeInProcessFeatureStart0StdDev",
            "TimeInProcessFeatureStart0P50",
            "TimeInProcessFeatureStart0P75",
            "TimeInProcessFeatureStart0P95",
            "TimeInProcessFeatureStart0",
            "TimeInProcessFeatureStart0P99"
		...
		var url = baseUrl + "?" + "granularity=" + granularity + "" + "&metrics=" + app.metrics.join() + 
					"&projects=" + project + 
    ...
    keysFromMetrics : function(metrics,keys) {

        var obj = {};
        _.each(keys,function(k){
            obj[k] = _.find( metrics, function(m) { return m.indexOf(k)!==-1;});
        })
    ...
    valuesFromDataPoint : function(datapoint) {

        var template = app.keysFromMetrics(app.metrics,app.valueKeys);

        var obj = _.mapValues( template, function(v) { 
            console.log(template,datapoint,v);
            return datapoint.data[v].value
        });
            ...
            chartData: {
                ...
                series : [ {
                    ...
                    data:  _.map(series, function(s) { 
                        var obj = app.valuesFromDataPoint(s);
                        return {
